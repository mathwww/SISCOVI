...
public class Retroatividade {
    ...
    public boolean ExisteRetroatividade(int pCodContrato, int pCodFuncaoContrato, int pMes, int pAno, int pOperacao ) throws SQLException {
        ...
        if(pOperacao == 1) {
            ...
            PreparedStatement preparedStatement = connection.prepareStatement("SELECT COUNT(RR.cod) FROM TB_RETROATIVIDADE_REMUNERACAO RR " +
                    "JOIN tb_remuneracao_fun_con RFC ON RFC.cod=RR.COD_REM_FUNCAO_CONTRATO WHERE RFC.COD_FUNCAO_CONTRATO=? AND ? >= DATEADD(DAY,1,EOMONTH(DATEADD(MONTH,-1, RR.INICIO))) AND ?<= RR.FIM");
        ...
        if(pOperacao == 2) {
            PreparedStatement preparedStatement = connection.prepareStatement("SELECT COUNT(rp.cod) FROM TB_RETROATIVIDADE_PERCENTUAL rp " +
                    "JOIN TB_PERCENTUAL_CONTRATO pc ON pc.cod=rp.cod_percentual_contrato WHERE pc.cod_contrato=? AND ? >= DATEADD(DAY,1,EOMONTH(DATEADD(MONTH,-1, RP.INICIO))) AND ?<= RP.FIM");
            preparedStatement.setInt(1,pCodContrato);
            preparedStatement.setDate(2, dataRef);
            preparedStatement.setDate(3,dataRef);
            rs = preparedStatement.executeQuery();
            ...
            preparedStatement = connection.prepareStatement("SELECT COUNT(RPE.COD) FROM tb_retro_percentual_estatico RPE" +
                    " WHERE RPE.COD_CONTRATO=? AND ? >= DATEADD(DAY, 1, EOMONTH(DATEADD(MONTH, -1, RPE.INICIO))) AND ? <= RPE.FIM");
            preparedStatement.setInt(1, pCodContrato);
            preparedStatement.setDate(2, dataRef);
            preparedStatement.setDate(3, dataRef);
            rs = preparedStatement.executeQuery();
